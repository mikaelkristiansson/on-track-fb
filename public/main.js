...
Demo.prototype.createMessage = function() {
  ...
  this.authenticatedRequest('POST', '/api/exercises', {type: 'gym'}).then(function(response) {
    this.$messageTextarea.val('');
    this.$messageTextarea.parent().removeClass('is-dirty');

    this.$createMessageResult.html('Created <b>' + response.created_at + '</b> type: ' + response.type);
...
Demo.prototype.listMessages = function(event) {
  ...
  var url = category ? '/api/exercises?date=2018' : '/api/exercises';
  this.authenticatedRequest('GET', url).then(function(response) {
    var elements = response.map(function(exercise) {
      return $('<li>')
        .text(exercise.created_at)
...
//   this.authenticatedRequest('GET', '/api/exercises/' + key).then(function(response) {
